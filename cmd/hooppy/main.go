...
func registerSearch(root *cobra.Command) {
	...
	postsCmd.Flags().IntVar(&sPhotosAmount, "photos-amount", 0, "photo count bucket. Measured against a live account to filter the result set (unset 10000; =1 → 9297; =5 → 566). The filters_plug values array is empty, so the valid keys are not discoverable from the descriptor; pass the integer key the vendor's UI selects.")
	postsCmd.Flags().IntVar(&sVideoDuration, "video-duration", 0, "video duration bucket. Measured against a live account (video content only): keys 1-4 are accepted and each changes the result set — unset 4194; =1 → 710; =2 → 159; =3 → 3525; =4 → 4036. The counts overlap (3 and 4 alone exceed the unfiltered total), so these are overlapping or cumulative ranges, not disjoint buckets. The vendor does not document the range semantics and the filters_plug values array is empty, so the meaning of each key is unknown — no labels (short/medium/long, etc.) are inferred.")
