...
* **Posts search / scraping (UNDOCUMENTED)**: `ListSearchPosts`, `ListSourceResources`, `GetParsingForm`, `StartParsing`, `StopParsing` via GET `/posts-search`, GET `/posts-search/source-resources`, GET `/posts-search/parsing/form`, POST `/posts-search/parsing/start`, DELETE `/posts-search/parsing`. Scrape posts from external social media pages (VK, Instagram, etc.) and filter by text, date, source. CLI: `hooppy search sources|posts|status|parse|stop`. MCP: `hooppy_list_search_posts`, `hooppy_list_source_resources`, `hooppy_parsing_status`, `hooppy_start_parsing`, `hooppy_stop_parsing`.
