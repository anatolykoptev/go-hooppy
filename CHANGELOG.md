...
* **Posts search / scraping (UNDOCUMENTED)**: `ListSearchPosts`, `ListSourceResources`, `GetParsingForm`, `StartParsing`, `StopParsing`, `CopySearchPost`, `RewriteSearchPost` via GET `/posts-search`, GET `/posts-search/source-resources`, GET `/posts-search/parsing/form`, POST `/posts-search/parsing/start`, DELETE `/posts-search/parsing`, PUT `/posts/copy`, PUT `/posts/rewrite`. Scrape posts from external social media pages (VK, Instagram, etc.) and filter by text, date, source. `CopySearchPost` copies a scraped post as-is; `RewriteSearchPost` lets you override the text and keep original photos via `--keep-photos` (downloads + re-uploads via `UploadMedia`). Both support immediate, scheduled (`when_type=2`), and by-schedule (`when_type=3`) publication. CLI: `hooppy search sources|posts|status|parse|stop|copy|rewrite`. MCP: `hooppy_list_search_posts`, `hooppy_list_source_resources`, `hooppy_parsing_status`, `hooppy_start_parsing`, `hooppy_stop_parsing`, `hooppy_copy_search_post`, `hooppy_rewrite_search_post`.
