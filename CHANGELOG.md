...
* **Posts search / scraping (UNDOCUMENTED)**: `ListSearchPosts`, `ListSourceResources`, `GetParsingForm`, `StartParsing`, `StopParsing`, `CopySearchPost` via GET `/posts-search`, GET `/posts-search/source-resources`, GET `/posts-search/parsing/form`, POST `/posts-search/parsing/start`, DELETE `/posts-search/parsing`, PUT `/posts/copy`. Scrape posts from external social media pages (VK, Instagram, etc.) and filter by text, date, source. `CopySearchPost` auto-fills text + photos from a scraped post via `search_post_id` — no need to manually construct the payload. CLI: `hooppy search sources|posts|status|parse|stop|copy`. MCP: `hooppy_list_search_posts`, `hooppy_list_source_resources`, `hooppy_parsing_status`, `hooppy_start_parsing`, `hooppy_stop_parsing`, `hooppy_copy_search_post`.
