...
* **Posts search / scraping (UNDOCUMENTED)**: `ListSearchPosts`, `ListSourceResources`, `GetParsingForm`, `StartParsing`, `StopParsing`, `CopySearchPost`, `RewriteSearchPost`, `GetSearchPostEdit` via GET `/posts-search`, GET `/posts-search/source-resources`, GET `/posts-search/parsing/form`, POST `/posts-search/parsing/start`, DELETE `/posts-search/parsing`, PUT `/posts/copy`, POST `/posts` (with `as_copy=1`), GET `/posts-search/{id}/edit`. Scrape posts from external social media pages (VK, Instagram, etc.) and filter by text, date, source, metrics (likes, views, comments, reposts, involvement) and content types. `ListSearchPosts` supports sorting (`sort_by`/`sort_direction`) and metric filters (`min_likes`/`min_views`/`min_comments`/`min_reposts`/`min_involvement`), content filters (`photos_amount`/`content_types`/`content_types_exclude`). `CopySearchPost` copies a scraped post as-is; `RewriteSearchPost` lets you override the text — attachments (photos, links, copyright, etc.) are preserved by default (downloads from edit endpoint URLs, re-uploads via `UploadMedia`, passes full MediaItem objects); use `--no-attachments` to strip them. `GetSearchPostEdit` returns a scraped post's data in re-publishable format. Attachment helpers: `LinkAttachment`, `SourceAttachment`, `CopyrightAttachment`, `TitleAttachment`, `PollAttachment`, `RepostAttachment`, `CommentAttachment`, `TelegramButtonsAttachment`, `SearchPostPhotos`, `SearchPostNonPhotoAttachments`. Types: `Poll`, `PollAnswer`, `Repost`, `Comment`, `TelegramButton`, `TelegramButtons`. CLI: `hooppy search sources|posts|status|parse|stop|copy|rewrite`. MCP: `hooppy_list_search_posts`, `hooppy_list_source_resources`, `hooppy_parsing_status`, `hooppy_start_parsing`, `hooppy_stop_parsing`, `hooppy_copy_search_post`, `hooppy_rewrite_search_post`.
