...
type CopySearchPostPayload struct {
	...
	SearchPostIDs       []int            `json:"search_post_ids,omitempty"`  // batch of scraped post IDs; when non-empty, wins over SearchPostID on Rewrite/Import (comma-joined in caller order). CopySearchPost ignores this — it uses the singular search_post_id wire field.
